dke2<-function (vec_data, ker, bw, x = NULL, a = 0, b = 1) 
{
  ...
  aux <- matrix(data = vec_data, nrow = length(vec_data), ncol = length(vec_data), 
                byrow = TRUE)
  aux2 <- kef2(x, aux, bw, ker, a, b)
  res <- apply(aux2, 1, mean)
  #C <- simp_int(x, res)
  fres<-function(x,t,bw,ker){
    auxiliar<-mean(kef2(x,t,bw,ker=ker,a=0,b=1))
    
    return(auxiliar)
  }
  
  C<-integrate(Vectorize(fres,"x"),0,Inf, t=datosGI,bw=b,ker=ker)$value
  result <- res/C
  result <- res/C
