...
estimadores.tiempo2<-function(muestra,L,ker,dist)
{
  ...
  if (x0[1]!=0) 
    {
      ...
      print("existe momentos")
  } else 
  {
      alfa.ini <- -1.5
      gama.ini <- mean(datosGI)*(-(alfa.ini)-1)
      print("No existe momentos")
  }
  #print(gama.ini)
  ...
  switch(dist,
         DT=salida.alfas<-data.frame(L=L,alfa.MV=MV[1],gama.MV=MV[2],alfa.DT=DosP[1],gama.DT=DosP[2],
                                     alfa.LC=alfa.LC,gama.LC=gama.LC,alfa.ini=alfa.ini,gama.ini=gama.ini))
