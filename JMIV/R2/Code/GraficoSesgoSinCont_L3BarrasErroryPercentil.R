...
medias0<-ddply(base.con.ecm,.(L,n,alfa),summarize,
               ...
               pi.MV.sesgo=quantile(sesgo.MV,0.025),
               ps.MV.sesgo=quantile(sesgo.MV,0.975),
               pi.GA.sesgo=quantile(sesgo.GA,0.025),
               ps.GA.sesgo=quantile(sesgo.GA,0.975),
               pi.LN.sesgo=quantile(sesgo.LN,0.025),
               ps.LN.sesgo=quantile(sesgo.LN,0.975),
               pi.LC.sesgo=quantile(sesgo.LC,0.025),
               ps.LC.sesgo=quantile(sesgo.LC,0.975),
               pi.MV.ecm=quantile(ecm.MV,0.025),
               ps.MV.ecm=quantile(ecm.MV,0.975),
               pi.GA.ecm=quantile(ecm.GA,0.025),
               ps.GA.ecm=quantile(ecm.GA,0.975),
               pi.LN.ecm=quantile(ecm.LN,0.025),
               ps.LN.ecm=quantile(ecm.LN,0.975),
               pi.LC.ecm=quantile(ecm.LC,0.025),
               ps.LC.ecm=quantile(ecm.LC,0.975))
...
graf.alfa<-paste("GraficoSesgoMVyGAyLNyLC_L=",L,"SinCont_BarrasErroryPercentil.eps",sep = "")
graf.ECM<-paste("GraficoECMMVyGAyLNyLC_L=",L,"SinCont_BarrasErroryPercentil.eps",sep = "")
...
p2<-pp1+geom_line(data = datos.grafico, aes(x = n, y = sesgo, color=metodo,linetype=metodo),size=2) +
...
####### ECM
cairo_ps(file = graf.ECM,onefile = FALSE,
         fallback_resolution=600,family="serif",width = 10, height = 10)

p3<-pp1+geom_line(data = datos.grafico, aes(x = n, y = ecm, color=metodo,linetype=metodo),size=2) +
