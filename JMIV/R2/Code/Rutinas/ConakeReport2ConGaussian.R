...
Conakereport2<-function (Vec, ker, h = NULL, nx,a = 0, b = 1) 
{
  if (missing(h)) {
    ...
    if (ker == "GA") {
      ...
      message("using gamma kernel 2 and h_n by cross validation technique.")
      return(list(hn_cv = h, C_n = bilan$C_n))
    }
    else if (ker == "BE") {
      ...
      message(paste("using Gaussian and h_n by cross validation technique."))
      return(list(hn_cv = h, C_n = bilan$C_n))
    }
  }
  else {
    ...
    if (ker == "GA") {
      ...
      message("with a gamma kernel 2 and a given bandwidth h=", 
              h)
      return(list(h = h, C_n = bilan$C_n))
    }
    else if (ker == "BE") {
      ...
      message("with a Gaussian kernel and a given bandwidth h=", 
              h)
      return(list(h = h, C_n = bilan$C_n))
    }
  }
